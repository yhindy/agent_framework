...
const debugLog = (location: string, message: string, data: any, hypothesisId: string) => {
  fetch('http://127.0.0.1:7246/ingest/72ee2966-a67f-4620-af62-58f554c3f942',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location,message,data,timestamp:Date.now(),sessionId:'debug-session',hypothesisId})}).catch(()=>{});
};
// #endregion
...
function initGlobalOutputListener() {
  ...
  // #region agent log
  debugLog('Terminal.tsx:global-init', 'Global listener initialized', {}, 'A');
  // #endregion
  ...
  window.electronAPI.onTerminalOutput((id, data) => {
    ...
    // #region agent log
    debugLog('Terminal.tsx:global-output', 'Global listener received output', { id, activeAgentId: activeTerminal?.agentId, willWriteLive: activeTerminal?.agentId === id, dataLen: data.length, cacheLen: outputCache.get(id)?.length }, 'A');
    // #endregion
